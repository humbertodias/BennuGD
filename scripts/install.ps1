# Install the latest BennuGD release into $HOME\bennugd and configure the user environment.
# Usage:
#   irm https://raw.githubusercontent.com/humbertodias/BennuGD/main/scripts/install.ps1 | iex
[CmdletBinding()]
param(
    [string]$Repo = $(if ($env:BENNUGD_REPO) { $env:BENNUGD_REPO } else { "humbertodias/BennuGD" }),
...
Write-Host "BennuGD installer" -ForegroundColor White
$tag = Resolve-Tag
$arch = Resolve-Arch
$platform = "windows"
$asset = "bennugd-$tag-$platform-$arch.zip"
